package vb.api.compare;

/**
 * Created by vbulba on 30.10.2014.
 */
public class UserData {

    public static final String ID="20495220";
    public static final String FIRST_NAME = "Владимир";
    public static final String LAST_NAME = "Бульба";
    public static final String MIDDLE_NAME = null;
    public static final String MIDDLE_NAME_NEW = "Александрович";
    public static final String EMAIL = "wbulba@mail.ru";
    public static final boolean IS_ADMIN = false;
    public static final boolean IS_APPLICANT=true;
    public static final boolean IS_EMPLOYER=false;
    public static final boolean IS_IN_SEARCH=true;
    public static final String RESUMES_URL="https://api.hh.ru/resumes/mine";
    public static final String NEGOTIATIONS_URL="https://api.hh.ru/negotiations";
}
